...
# A failure after bg_init removes the half-built handle and its directory again,
# so the command can simply be re-run.
scratch_path <- tempfile("glade-prototype-wipe-me-")
tryCatch({
  ...
  if (exists("glade_prototype", envir = .GlobalEnv, inherits = FALSE)) rm(glade_prototype, envir = .GlobalEnv)
  unlink(scratch_path, recursive = TRUE)
  stop(e)
})
