...
glade_read_handle <- function(handle) {
  ...
  list(guide = guide, snapshot = snapshot, summaries = summaries, token = token)
}

# Pure projection of one read into the contract shape; makes no Bayesgrove calls.
glade_project_read <- function(data) {
  guide <- data$guide
...
# `read` and `project` are injectable so the decide paths can be exercised without
# disturbing a real Bayesgrove handle. The response discriminates the mutation
# outcome from the projection: a failed refresh after a recorded decision returns
# `kind = "decided"` with no snapshot instead of failing the whole request.
glade_review_request <- function(handle, request, read = glade_read_handle, project = glade_project_read) {
  data <- read(handle)
  if (identical(request$kind, "decide")) {
    if (!identical(request$token, data$token)) stop("Evidence changed. Refresh and review it before recording a decision.")
    action <- if (is.character(request$action_id) && length(request$action_id) == 1) data$guide$actions[[request$action_id]] else NULL
    ...
    # The decision is already recorded, so only the refreshed projection is
    # allowed to fail here, and only softly: the extension keeps its previous
    # snapshot and shows `refresh_error` instead of losing the acknowledgement.
    refreshed <- tryCatch(project(read(handle)), error = function(e) conditionMessage(e))
    if (is.character(refreshed)) {
      return(list(kind = "decided", snapshot = NULL, refresh_error = refreshed))
    }
    return(list(kind = "decided", snapshot = refreshed, refresh_error = NULL))
  }
  list(kind = "snapshot", snapshot = project(data))
}
