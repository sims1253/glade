...
- Retired the standalone Electron client: removed the desktop, server, web, and marketing apps, the contracts and shared packages, their build, smoke, and release tooling, and the legacy Bayesgrove 0.5.1 CI pin. The client needed the `bg_serve()` interface Bayesgrove removed in 0.6.0; its reference value lives in git history (#22).
- Recorded the rebuild decision in RESTART.md (application layer as a Positron extension, Bayesgrove as the workflow engine, retired client's reference preserved in git history) and rewrote the README around the restart state.
- Added vendored anti-slop oxlint rules for TypeScript sources, with `lint:strict` to run them without exceptions.
...
- Fixed the Positron prototype to key persisted drafts by `[sessionId, handle]` so a draft survives reattaching to the same handle, and to validate a restored `draft.choice` against the refreshed `choices` (#11).
...
- Hardened the prototype R bridge so a single missing field no longer fails the whole snapshot decode: `fallback()` now covers every projected string field, string arrays such as `choices` always serialize as `[]` even when the underlying field is missing, and summary records without a `summary_id` are dropped instead of blanking the panel (#8).
...
- Isolated the prototype `bun run dev` host onto its own profile under the system temp directory via `--user-data-dir`/`--extensions-dir` and declared the missing `onCommand:gladePrototype.demo` activation event (#17).
- Mapped the prototype's failure causes to readable notice text instead of raw internal dumps: string failures pass through, host `Error` messages are unwrapped from Effect's `UnknownException` (missing `bridge.R` and busy-session rejections get dedicated copy), and `HandleName` or response decode failures show short friendly messages while their parse trees go to the extension host log (#7).
- Separated the prototype's mutation outcome from its projection so a recorded decision can no longer be masked by a failed refresh: `glade_review_request` returns a `kind`-discriminated response, and when the post-decision projection fails it returns `kind = "decided"` with the snapshot absent and a `refresh_error` message, so the extension keeps the previous snapshot displayed, shows a decision-recorded-but-refresh-failed notice, and never re-issues the decide (#9).
- Made the prototype's busy state honest: notices render alongside the busy indicator instead of being hidden by the waiting text, webview messages arriving during an in-flight request get a previous-request-still-in-progress notice instead of being silently dropped, and the whole busy region including its first render sits inside the exit guard so `busy` resets on every exit path including defects (#10).
- Rebuilt the prototype panel as a persistent document: the extension ships a static shell once and delivers every later state as a `postMessage` payload that a boot script applies to the DOM in place, so refreshes and decisions keep keyboard focus and scroll position, the `role="status"` notice node survives to be announced by screen readers (with unchanged notices re-announced), and an in-progress decision form is only rebuilt when the selected review, its evidence token, or a fresh decision completion — carried as a consume-once decision id so replays never clear a newer draft — calls for it (#13).
