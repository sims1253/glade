...
- Rebuilt the prototype panel as a persistent document: the extension ships a static shell once and delivers every later state as a `postMessage` payload that a boot script applies to the DOM in place, so refreshes and decisions keep keyboard focus and scroll position, the `role="status"` notice node survives to be announced by screen readers, and an in-progress decision form is only rebuilt when the selected review or its evidence token changed (#13).
